...
func onSetAttr(ctx context.Context, w *response, userHandle Handler) error {
	...
	if err != nil {
		Log.Errorf("SETATTR ReadSetFileAttributes error: %v", err)
		return &NFSStatusError{NFSStatusInval, err}
	}
	...
	if guard, err := xdr.ReadUint32(w.req.Body); err != nil {
		Log.Errorf("SETATTR guard read error: %v", err)
		return &NFSStatusError{NFSStatusInval, err}
	} else if guard != 0 {
		// read the ctime.
		t := FileTime{}
		if err := xdr.Read(w.req.Body, &t); err != nil {
			Log.Errorf("SETATTR ctime read error: %v", err)
			return &NFSStatusError{NFSStatusInval, err}
		}
		attr := ToFileAttribute(info, fullPath)
		Log.Debugf("SETATTR guard check: client=%d/%d server=%d/%d match=%v path=%s",
			t.Seconds, t.Nseconds, attr.Ctime.Seconds, attr.Ctime.Nseconds, t == attr.Ctime, fullPath)
		if t != attr.Ctime {
			Log.Warnf("SETATTR guard MISMATCH: client=%d/%d server=%d/%d path=%s",
				t.Seconds, t.Nseconds, attr.Ctime.Seconds, attr.Ctime.Nseconds, fullPath)
